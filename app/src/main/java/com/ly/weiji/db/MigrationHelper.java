...
import com.ly.weiji.db.greendao.AccountDao;
...
/**
 * greenDao使用的helper，主要是重写了onUpgrade方法，当数据库版本发生变化时，便于数据库数据迁移
 * <p>
 * 更新流程：
 ...
 * <p>
 * Created by ly on 2017/2/24 14:40.
 */
...
public class MigrationHelper extends DaoMaster.DevOpenHelper {
    ...
    public void onUpgrade(Database db, int oldVersion, int newVersion) {
        Logger.d("---->执行数据库升级: from " + oldVersion + " to:" + newVersion);
        if (newVersion > oldVersion) {
            migrate(db, AccountDao.class);
        } else {
            Logger.e("Can't downgrade database from version" + oldVersion + " to" + newVersion);
        }
    }
